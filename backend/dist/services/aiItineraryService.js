...
const AICallLog_1 = __importDefault(require("../models/AICallLog"));
...
const destinationPricing_1 = require("../config/destinationPricing");
class AIItineraryService {
    constructor() {
        ...
        this.weatherService = weatherService_1.default;
    }
    /**
     * Generate AI-powered itinerary
     */
    async generateItinerary(params, userId, itineraryId) {
...
Provide detailed, practical, and personalized travel itineraries in JSON format. Include specific timing, realistic costs, exact locations, and insider knowledge that goes beyond typical tourist information.`
    ...
    async fetchWeatherForItinerary(structuredItinerary, params) {
        try {
            ...
            const weatherPromises = Array.from(locations).map(async (location) => {
                try {
                    ...
                    const weatherData = await this.weatherService.getWeatherForecast(coordinates.lat, coordinates.lng, startDate, endDate);
            ...
            const aggregated = await Promise.all(validWeatherData.map(async ({ location, coordinates, forecast }) => {
                ...
                const allMins = forecast.map((d) => d.temperature?.min ?? 0);
                const allMaxs = forecast.map((d) => d.temperature?.max ?? 0);
                ...
                const avgPrecip = Math.round((forecast.reduce((acc, d) => acc + (d.precipitation || 0), 0) / forecast.length));
                // Aggregate recommendations and pick unique ones (limit to 5)
                const recommendationsSet = new Set();
                forecast.flatMap((d) => d.recommendations || []).forEach((r) => { if (r)
                    recommendationsSet.add(String(r).trim()); });
    ...
    async saveExtractedData(generatedItinerary, itineraryId, userId, params) {
        try {
            ...
            if (generatedItinerary.accommodationSuggestions && Array.isArray(generatedItinerary.accommodationSuggestions)) {
                const accommodations = generatedItinerary.accommodationSuggestions.map((acc) => ({
                    userId,
                ...
                if (accommodations.length > 0) {
                    ...
                    if (bulkOps.length > 0) {
                        await Accommodation_1.default.bulkWrite(bulkOps);
            ...
            if (generatedItinerary.restaurantRecommendations && Array.isArray(generatedItinerary.restaurantRecommendations)) {
                const restaurants = generatedItinerary.restaurantRecommendations.map((rest) => ({
                    userId,
                ...
                if (restaurants.length > 0) {
                    ...
                    if (bulkOps.length > 0) {
                        await Restaurant_1.default.bulkWrite(bulkOps);
            ...
            if (generatedItinerary.dayPlans && Array.isArray(generatedItinerary.dayPlans)) {
                ...
                if (places.length > 0) {
                    await Place_1.default.insertMany(places);
                }
            }
    ...
    buildEnhancedPrompt(params) {
        ...
        const pricingGuidance = (0, destinationPricing_1.formatPricingGuidance)(primaryDestination, params.budget, numberOfRooms, totalPeople);
        // Generate unique trip name based on location sequence
        const generateTripName = (source, destinations) => {
...
- Authentic local experiences`;
    ...
    validateAndStructureResponse(response, params) {
        // Use the robust parser
        const parseJsonOrArray = (d) => this.parseJsonOrArrayInternal(d);
        // Generate unique trip name based on location sequence
        const generateTripName = (source, destinations) => {
            const allLocations = [source, ...destinations, source]; // source -> destinations -> source
            const locationNames = allLocations.map(loc => {
        ...
        const processedDayPlans = response.dayPlans.map((day, index) => {
            ...
            const processedDay = {
                day: day.day || index + 1,
                morning: morningArray.map((activity) => normalizeActivity(activity)),
                afternoon: afternoonArray.map((activity) => normalizeActivity(activity)),
                evening: eveningArray.map((activity) => normalizeActivity(activity)),
        ...
        const processedAccommodations = rawAccommodations.map((acc, idx) => {
            if (typeof acc === 'string') {
                ...
                if (parsed.length > 0 && typeof parsed[0] === 'object')
                    ...
                    return { name: acc, type: 'Hotel', priceRange: '', location: '', amenities: [] };
            }
            // Ensure amenities is always an array of strings
            let amenitiesArray = [];
            if (Array.isArray(acc.amenities)) {
                amenitiesArray = acc.amenities.map((a) => String(a));
            }
        ...
        const processedTransportationTips = rawTransportation.map((tip) => {
        ...
        const processedRestaurants = rawRestaurants
            ...
            if (dietType === 'veg') {
                return rest.dietaryOptions?.some((option) => option.toLowerCase().includes('veg') || option.toLowerCase().includes('vegetarian') || option.toLowerCase().includes('vegan'));
            }
            else if (dietType === 'non-veg') {
                return rest.dietaryOptions?.some((option) => option.toLowerCase().includes('non-veg') || option.toLowerCase().includes('meat') || option.toLowerCase().includes('seafood'));
            }
        ...
        const processedDailyCostBreakdown = rawDailyCostBreakdown.map((day) => ({
            day: day.day || 0,
        ...
        if (processedDailyCostBreakdown.length > 0) {
            processedDayPlans.forEach((dayPlan) => {
                const costBreakdown = processedDailyCostBreakdown.find((db) => db.day === dayPlan.day);
                if (costBreakdown) {
        ...
        let calculatedBudgetBreakdown = undefined;
        ...
        if (processedDailyCostBreakdown.length > 0) {
            ...
            calculatedTotalCost = Object.values(totals).reduce((sum, cost) => sum + cost, 0);
        }
     ...
     */
    async regenerateDay(source, destinations, // Changed from destination string to destinations array
    travelMode, dayNumber, params) {
...
Return JSON:
{
  ...
  "notes": ""
     ...
     */
    async getSuggestions(itinerary, userEdit) {
        try {
...
Return JSON: { "suggestions": ["suggestion1", "suggestion2", ...] }`;
    ...
    async testFetchWeather(params) {
        return this.fetchWeatherForItinerary({}, params);
    }
    async generateSeasonalSummaryForLocation(location, startDate, endDate) {
        try {
...
exports.default = new AIItineraryService();
