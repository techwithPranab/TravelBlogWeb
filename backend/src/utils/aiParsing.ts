...
export const deepParseIfString = (data: any): any => {
  const tryParseString = (str: string): any => {
    ...
    try {
      const replaced = s.replace(/'([^'\\]*(?:\\.[^'\\]*)*)'/g, (_m: string, g1: string) => `"${g1.replace(/"/g, '\\"')}"`)
      ...
      if (typeof el === 'string') {
        ...
        if (parsed !== undefined) {
          if (Array.isArray(parsed)) {
            out.push(...parsed.map(item => deepParseIfString(item)))
          } else {
            out.push(deepParseIfString(parsed))
          }
        } else {
          out.push(el)
        }
      } else {
        out.push(deepParseIfString(el))
      }
    }
    return out
  }
  ...
  if (data && typeof data === 'object') {
    ...
    for (const key of Object.keys(data)) {
      result[key] = deepParseIfString((data as any)[key])
    }
    return result
  }
