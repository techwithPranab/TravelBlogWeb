...
class PDFService {
   ...
   */
  async generateItineraryPDF(itinerary: any): Promise<Buffer> {
  ...
  private addHeader(doc: PDFKit.PDFDocument, itinerary: any) {
    doc
      .fontSize(24)
      .fillColor('#4F46E5')
  ...
  private addTripOverview(doc: PDFKit.PDFDocument, itinerary: any) {
    doc
      .fontSize(16)
      .fillColor('#1F2937')
  ...
  private addDayPlans(doc: PDFKit.PDFDocument, itinerary: any) {
    doc
      .fontSize(16)
      .fillColor('#1F2937')
      .font('Helvetica-Bold')
    ...
    itinerary.dayPlans?.forEach((day: any, index: number) => {
  ...
  private addAccommodations(doc: PDFKit.PDFDocument, itinerary: any) {
    ...
    itinerary.accommodationSuggestions.forEach((acc: any) => {
      doc
        .fontSize(12)
        .fillColor('#4F46E5')
        .font('Helvetica-Bold')
  ...
  private addRestaurants(doc: PDFKit.PDFDocument, itinerary: any) {
    if (doc.y > 600) doc.addPage()
    ...
    itinerary.restaurantRecommendations.forEach((rest: any) => {
      doc
        .fontSize(12)
        .fillColor('#4F46E5')
        .font('Helvetica-Bold')
  ...
  private addTransportation(doc: PDFKit.PDFDocument, itinerary: any) {
    if (doc.y > 600) doc.addPage()
    ...
    itinerary.transportationTips.forEach((tip: any) => {
  ...
  private addBudgetBreakdown(doc: PDFKit.PDFDocument, itinerary: any) {
    if (doc.y > 600) doc.addPage()
  ...
  private addGeneralTips(doc: PDFKit.PDFDocument, itinerary: any) {
    if (doc.y > 600) doc.addPage()
    ...
    itinerary.generalTips.forEach((tip: string) => {
  ...
  private addPackingList(doc: PDFKit.PDFDocument, itinerary: any) {
    if (doc.y > 600) doc.addPage()
  ...
  private addFooter(doc: PDFKit.PDFDocument) {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc
        ...
        .fillColor('#9CA3AF')
        .text(
          `Generated by TravelBlog AI | Page ${i + 1} of ${pageCount}`,
          50,
          doc.page.height - 50,
          { align: 'center' }
        )
