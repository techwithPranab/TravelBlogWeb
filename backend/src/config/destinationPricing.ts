...
export interface CountryPricing {
  ...
  transportation: {
    localTransportPerDay: { min: number; max: number }
    airportTransfer: { min: number; max: number }
    intraCityTaxi: { min: number; max: number }
    publicTransport: { min: number; max: number }
  }
  food: {
    budget: { breakfast: number; lunch: number; dinner: number }
    moderate: { breakfast: number; lunch: number; dinner: number }
    luxury: { breakfast: number; lunch: number; dinner: number }
  }
  averageAttractionCost: { min: number; max: number }
}

// Destination pricing database (all costs in INR)
export const DESTINATION_PRICING: { [key: string]: CountryPricing } = {
  // India
  'India': {
    country: 'India',
    currency: 'INR',
...
export const CITY_TO_COUNTRY_MAP: { [key: string]: string } = {
  // India
...
export function getCountryFromDestination(destination: string): string {
  ...
  for (const country of Object.keys(DESTINATION_PRICING)) {
...
export function getDestinationPricing(
  ...
  return DESTINATION_PRICING[country] || DESTINATION_PRICING['Default']
}

/**
 * Format pricing guidance for AI prompt
 */
export function formatPricingGuidance(
  destination: string,
  budgetLevel: 'budget' | 'moderate' | 'luxury',
